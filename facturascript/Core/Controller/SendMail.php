...
class SendMail extends Controller
{
    ...
    public function url(): string
    {
        ...
        #INI MOD ERICK
		if (!is_null($this->request->get('fileNameE'))) {
	        $sendParams = ['fileNameE' => $this->request->get('fileNameE', '')];
    	    if (empty($sendParams['fileNameE'])) {
        	    return parent::url();
        	}
		}
    ...
    protected function removeOld(): void
    {
		...
		#INI MOD ERICK
		foreach (glob(FS_FOLDER . '/MyFiles/XMLMail/*_mail_*.xsig') as $fileNameE) {
            $partsE = explode('_', $fileNameE);
            $timeE = (int)substr(end($partsE), 0, -4);
            if ($timeE < (time() - self::MAX_FILE_AGE)) {
                unlink($fileNameE);
            }
        }
		#FIN MOD ERICK
    ...
    protected function send(): bool
    {
        ...
        if ($this->newMail->send()) {
			...
			#INI MOD ERICK
			$fileNameE = $this->request->get('fileNameE', '');
            if (file_exists(FS_FOLDER . '/MyFiles/XMLMail/' . $fileNameE)) {
                unlink(FS_FOLDER . '/MyFiles/XMLMail/' . $fileNameE);
            }
			#FIN MOD ERICK
            return true;
        }
    ...
    protected function setAttachment(): void
    {
		...
		if (!is_null($this->request->get('fileNameE'))) {
			$fileNameE = $this->request->get('fileNameE', '');
    	    $this->newMail->addAttachment(FS_FOLDER . '/MyFiles/XMLMail/' . $fileNameE, $fileNameE);
		}
		#FIN MOD ERICK
    }
