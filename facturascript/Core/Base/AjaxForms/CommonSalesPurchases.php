...
trait CommonSalesPurchases
{
    ...
    protected static function fastLineInput(Translator $i18n, BusinessDocument $model, string $jsName): string
    {
        return $model->editable ? '<div class="col-8 col-md">'
            ...
            . '<div class="input-group-prepend"><span class="input-group-text btn" style="cursor:auto;"><i class="fas fa-barcode"></i></span></div>'
            . '<input type="text" name="fastli" class="form-control h-100" placeholder="' . $i18n->trans('barcode')
    ...
    protected static function fecha(Translator $i18n, BusinessDocument $model, bool $enabled = true): string
    {
        ...
        return empty($model->subjectColumnValue()) ? '' : '<div class="col-sm">'
            ...
            . '<input type="date" ' . $attributes . ' value="' . date('Y-m-d', strtotime($model->fecha)) . '" class="form-control h-100"/>'
            . '</div>'
            . '</div>';
    }
    ...
    protected static function femail(Translator $i18n, BusinessDocument $model): string
    {
        ...
        return '<div class="col-sm-6">'
            ...
            . '<input type="date" ' . $attributes . ' value="' . $value . '" class="form-control h-100" style="max-height:40.3px;"/>'
            . '</div>'
            . '</div>';
    }
    ...
    protected static function hora(Translator $i18n, BusinessDocument $model): string
    {
        ...
        return empty($model->subjectColumnValue()) ? '' : '<div class="col-sm-6">'
            ...
            . '<input type="time" ' . $attributes . ' value="' . date('H:i:s', strtotime($model->hora)) . '" class="form-control h-100" style="max-height:40.3px"/>'
            . '</div>'
            . '</div>';
    }
    ...
    {
		$status_config = FS_CHANGE_DISABLED_STATUS_FACTURAS; #MOD ERICK
       	$estadoFacturaEmitida = FS_STATUS_FACTURA_EMITIDA; #MOD ERICK
		...
		if (in_array($model->modelClassName(), ['FacturaCliente'])) {
			if (in_array($model->idestado, [$estadoFacturaEmitida]) && $status_config) {
				$disabled = 'disabled="'.$status_config.'" ';
			}		
		}
		#FIN MOD ERICK

        $status = $model->getStatus();
        $btnClass = 'btn btn-block btn-bblue btn-spin-action';
        ...
        return '<div class="col-sm-auto">'
            ...
            . '<div class="dropdown">'
            . '<button class="' . $btnClass . ' dropdown-toggle" '.$disabled.' type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">'
            . '<i class="' . static::idestadoIcon($status) . ' fa-fw"></i> ' . $status->nombre
            . '</button>'
    ...
    protected static function productBtn(Translator $i18n, BusinessDocument $model): string
    {
        return $model->editable ? '<div class="col-9 col-md col-lg-2">'
            . '<div class="input-group mb-3">'
            . '<input type="text" id="findProductInput" class="form-control h-100" placeholder="' . $i18n->trans('reference') . '"/>'
    ...
    protected static function total(Translator $i18n, BusinessDocument $model, string $jsName): string
    {
        return empty($model->total) ? '' : '<div class="col-sm"><div class="form-group">' . $i18n->trans('total')
            ...
            . '" class="form-control h-100" disabled/>'
    ...
    {
        return $model->subjectColumnValue() && $model->editable ? '<a href="' . $model->url() . '" class="btn btn-bblue mr-2">'
