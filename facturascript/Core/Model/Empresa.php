...
class Empresa extends Base\Contact
{
    ...
    {
        // Limitar el número máximo de empresas según configuración
        if (empty($this->idempresa)) {
            $maxEmpresasConfig = Tools::settings('suscription', 'empresas');
            if ($maxEmpresasConfig !== "Ilimitadas" || $maxEmpresasConfig != "Ilimitados") {
                $maxEmpresas = (int)$maxEmpresasConfig;
                $count = self::$dataBase->select("SELECT COUNT(*) as total FROM " . self::tableName())[0]['total'];
                if ($count >= $maxEmpresas) {
                    Tools::log()->warning('Número máximo de empresas alcanzado (' . $maxEmpresas . ')');
                    return false;
                }
            }
        }
    ...
    protected function saveInsert(array $values = []): bool
    {
        ...
        if ($usuario->nick != "admin") {
            Tools::log()->warning('No puede crear empresas.');
            return false;
        }
