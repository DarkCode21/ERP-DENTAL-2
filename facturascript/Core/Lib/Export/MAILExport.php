...
use FacturaScripts\Dinamic\Lib\Export\XMLExport; #MOD ERICK
...
class MAILExport extends ParentClass
{
    ...
    {
        #INI MOD ERICK
		$xmlDoc = new XMLExport();
		$xmlDoc->newDoc($this->getFileName(), $this->sendParams['modelCode']);
		#FIN MOD ERICK
		...
		#INI MOD ERICK
		$fileNameE = null;
		if (!is_null($xmlDoc->xmlfacturae->idfactura)) {
			$fileNameE = $xmlDoc->getFileName() . '_mail_' . time() . '.xsig';
		}	
		#FIN MOD ERICK
		...
		#INI MOD ERICK
		if (!is_null($fileNameE)) {
	 		$filePathE = \FS_FOLDER . '/MyFiles/XMLMail/' . $fileNameE;
    	    if (false === \file_put_contents($filePathE, $xmlDoc->getDoc())) {
        	    $this->toolBox()->i18nLog()->error('folder-not-writable');
				return;
        	}
			$this->sendParams['fileNameE'] = $fileNameE;
       	}
        #FIN MOD ERICK
