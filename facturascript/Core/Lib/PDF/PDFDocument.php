...
abstract class PDFDocument extends PDFCore
{
    ...
    protected function insertHeader($idempresa = null)
    {
        ...
        // Agrupar de 2 en 2
        $contactLines = [];
        for ($i = 0; $i < count($contactData); $i += 2) {
            $line = $contactData[$i];
            if (isset($contactData[$i + 1])) {
                $line .= ' · ' . $contactData[$i + 1];
            }
            $contactLines[] = $line;
        }
    ...
    {
        if (empty($qrImage) || $rightBlockWidth <= 0) {
            return;
        }
        ...
        // Posición del QR centrado en el bloque derecho
        $qrX = $startX + $leftBlockWidth + 10 + ($availableWidth - $qrSize) / 2;
        $qrY = $startY - 10;

        // Detectar si $qrImage es base64
        if (str_starts_with($qrImage, 'data:image/')) {
            $base64Data = explode(',', $qrImage, 2)[1] ?? '';
        ...
        // Texto fijo encima y debajo del QR
        $fontSize = self::FONT_SIZE;
        $lineHeight = $fontSize + 2;

        // Texto encima
        $this->pdf->addText($qrX + $qrSize / 2, $qrY + 5, $fontSize, 'QR tributario', 0, 'center');

        // Texto debajo
        $this->pdf->addText($qrX + $qrSize / 2, $qrY - $qrSize - $lineHeight, $fontSize, 'Factura verificable en la sede electrónica de la AEAT', 0, 'center');

        // Ajustar posición Y del PDF
        $this->pdf->y = $qrY - $qrSize - ($lineHeight * 2) - 10;
    }
