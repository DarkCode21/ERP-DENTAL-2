...
trait DocFilesTrait
{
    ...
    private function addFileAction(): bool
    {
        ...
        // SOPORTE PARA FIRMA DE CLIENTE (canvas base64)
        $type = $this->request->request->get('type-file');
        // Definir FS_FOLDER si no está definida
        if (!defined('FS_FOLDER')) {
            define('FS_FOLDER', dirname(__DIR__, 3));
        }
        if ($type == 'canvas') {
            $uploadImage = $this->request->request->get('new-file');
            $uploadImage = str_replace('data:image/png;base64,', '', $uploadImage);
            $uploadImage = str_replace(' ', '+', $uploadImage);
            $uploadFile = base64_decode($uploadImage);
            if ($uploadFile && $this->moveSignature(FS_FOLDER . DIRECTORY_SEPARATOR . 'MyFiles', 'firma-cliente.png', $uploadFile)) {
                $newFile = new AttachedFile();
                $newFile->path = 'firma-cliente.png';
                ...
                $fileRelation->observations = 'Firma de Cliente';
    ...
    // Copiado de Core_old: guardar la firma como archivo
    private function moveSignature($folder, $filename, $data)
    {
        if (!is_dir($folder)) {
