 ...
 * @author Rafael del Pozo Barajas <pozo@us.es>
 * Añade la columna saldo a las partidas del mayor.
 * Permite calcular el saldo de cada partida en la subcuenta mostrada.
 * Cambia el orden inicial de las partidas a número de asiento y fecha ascendentes
 */

namespace FacturaScripts\Plugins\SaldosMayor\Controller;

use FacturaScripts\Plugins\SaldosMayor\Lib\Accounting\Ledger;
...
class EditSubcuenta extends EditController
{
    ...
    protected function createDepartureView(string $viewName = 'ListPartidaAsiento')
    {
        ...
        $this->views[$viewName]->addOrderBy(['fecha', 'numero'], 'fecha', 1);
        $this->views[$viewName]->addOrderBy(['numero', 'fecha'], 'numero', 1);
    ...
    //Actuliza el saldo de cada partida de la subcuenta y lo guarda en la base de datos
    protected function actualizaSaldosSubcuenta($idSubAccount)
    {
        $sql = 'SELECT asientos.numero, asientos.fecha, partidas.codsubcuenta,'
            . ' partidas.debe, partidas.haber, partidas.idpartida'
            . ' FROM partidas'
                . ' LEFT JOIN asientos ON partidas.idasiento = asientos.idasiento'
            . ' WHERE partidas.idsubcuenta = ' . $idSubAccount
        ...
        foreach ($Partidas as $Partida) {
            ...
            $idPartida = (int)$Partida['idpartida'];
            $this->dataBase->exec("UPDATE partidas SET saldo = $Saldo WHERE idpartida=$idPartida");
        }        
    ...
    protected function loadData($viewName, $view)
    {
        ...
        switch ($viewName) {
            case 'ListPartidaAsiento':
                ...
                $this->actualizaSaldosSubcuenta ($idsubcuenta);  //Esto hace que se recalculen los saldos de la subcuenta antes de visualizarse
                $view->loadData('', $where);
                if ($view->count > 0) {
