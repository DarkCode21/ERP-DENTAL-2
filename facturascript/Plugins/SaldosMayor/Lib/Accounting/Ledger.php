...
namespace FacturaScripts\Plugins\SaldosMayor\Lib\Accounting;
...
class Ledger extends AccountingBase
{
        ...
        switch ($params['grouped']) {
            ...
            default:
                ...
                foreach ($this->getData($params) as $line) {
                    if ($Asiento != $line['numero']) {
                        if ($Asiento!="" && $params['format']=='XLS')
                            $ledger['lines'][] = [];
                        $Asiento = $line['numero'];
                    }
                    $debe += (float)$line['debe'];
    ...
    protected function getData(array $params = []): array
    {
        ...
        $sql = 'SELECT asientos.numero, asientos.fecha, partidas.codsubcuenta,'
            . ' partidas.concepto, partidas.debe, partidas.haber, partidas.saldo,' //PUESTO
    ...
    protected function processLine(&$ledger, $line)
    {
        $ledger[] = [
            ...
            'saldo' => $this->toolBox()->coins()->format($line['saldo'], FS_NF0, '')  //PUESTO
        ];
    }
