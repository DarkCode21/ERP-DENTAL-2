...
class Backup extends Controller
{
    ...
    protected function createZipAction(): void
    {
        ...
        $base_name = date('Y-m-d_H-i-s');
        $sql_file_name = $base_name . '.sql';
        SimpleBackup::setDatabase([FS_DB_NAME, FS_DB_USER, FS_DB_PASS, FS_DB_HOST])
            ->storeAfterExportTo($folder, $sql_file_name);

        $sql_file_path = Tools::folder('MyFiles', 'Backups', $sql_file_name);
        if (false === file_exists($sql_file_path)) {
            Tools::log()->error('record-save-error');
            return;
        }
        ...
        if (filesize($zip_file_path) === 0) {
            Tools::log()->warning('backup-empty-warning');
        }

        $this->zip_file_name = $base_name . '.zip';
        Tools::log()->notice('file-ready-to-download');
    }
    ...
    private function defaultChecks(): void
    {
        ...
        if ($folderMb >= $memoryMb) {
            Tools::log()->warning('backup-memory-warning', [
                '%size%' => $folderMb,
                '%memory%' => $memoryMb,
                '%empresa%' => $this->empresa->nombrecorto
            ]);
        }
    }
