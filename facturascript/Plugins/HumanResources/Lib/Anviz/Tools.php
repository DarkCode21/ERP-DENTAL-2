...
class Tools {
    ...
    public static function createRequestString(int $deviceId, int $command, string $data = '')
    {
        $req = sprintf(
            "a5%08s%02x%04x%s",
            $deviceId,
            $command,
            empty($data) ? 0 : strlen($data) / 2,
            $data
        );
        $req .= static::crc16(hex2bin($req));
        return hex2bin($req);
    }
    ...
    private static function crc16(string $input): string
    {
        $crc = 0xFFFF;
        ...
        $result = sprintf("%04s", strtoupper(dechex($crc)));
