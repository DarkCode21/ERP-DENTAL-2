 ...
 */
class PrePagoCli extends ModelOnChangeClass
{
    use ModelTrait;
    ...
    public function getCustomer(): ?Cliente
    {
        ...
        if (!empty($this->codcliente) && $customer->loadFromCode($this->codcliente)) {
            return $customer;
        }
    ...
    public function test(): bool
    {
        ...
        if ($document && $document->exists() && empty($this->codcliente)) {
            $this->codcliente = $document->codcliente;
        }
        ...
        if (empty($this->codcliente)) {
            Tools::log()->warning('customer-not-assigned');
            return false;
        }
    ...
    public function url(string $type = 'auto', string $list = 'List'): string
    {
        ...
        if ($customer && $customer->exists() && $type === 'list') {
            return $customer->url();
        }
    ...
    protected function onChange($field)
    {
        ...
        if (in_array($field, ['amount', 'codpago', 'payment_date'])) {
            ...
            $document = $this->getDocument();
            if ($document && $document->exists()) {
                PrePagoCliToAccounting::generate($this->id, false);
            }
        }
    ...
    protected function saveInsert(array $values = []): bool
    {
        ...
        $document = $this->getDocument();
        if ($document && $document->exists()) {
            PrePagoCliToAccounting::generate($this->id, true);
        }
