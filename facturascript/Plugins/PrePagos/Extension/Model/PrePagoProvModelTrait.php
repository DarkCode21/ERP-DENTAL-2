...
trait PrePagoProvModelTrait
{
    ...
    public function getPayments(): Closure
    {
        return function () {
            ...
            return PrePagoProv::all($where, [], 0, 0);
        };
    }
    ...
    public function onChange(): Closure
    {
        return function ($field) {
            // si cambió el proveedor, actualizamos el proveedor de los prepagos
            if ($field === 'codproveedor') {
                foreach ($this->getPayments() as $payment) {
                    $payment->codproveedor = $this->codproveedor;
                    $payment->save();
                }
            }
        ...
        return function () {
            ...
            foreach ($this->getAvailableStatus() as $status) {
                ...
                if (false === $this->previousData['editable'] && $this->editable && empty($status->generadoc)) {
                    PrePagoProvToAccounting::regenerate([$this]);
                    break;
                }
