...
/**
 * Class for the generation of accounting entry for finalize amortization.
 *
 * @author Jose Antonio Cuello Principal <yopli2000@gmail.com>
 */
class AmortizationFinalizeToAccounting extends AccountingClass
{
    ...
    /**
     * Contabilize the final of the amortization.
     *
     * @param Amortizacion $model
     * @param string $endDate
     * @return bool
     */
    public static function exec(Amortizacion $model, string $endDate): bool
    {
        $accounting = new self();
        $accounting->endDate = $endDate;
    ...
    public function generate($model): bool
    {
        ...
        try {
            ...
            // link account entry with document
     ...
     *   - calculate amortization and pending amortization.
     *   - add lines to account entry.
     *
     * @param Asiento $entry
     * @return bool
     */
    protected function addLines(Asiento &$entry): bool
    {
        ...
        $pendingAmortization = round($this->document->valor - $totalAmortized, FS_NF0);

        if (false === $this->addLineToEntry($entry, $this->document->codsubcuentacierre, false, $this->document->valor)) {
            return false;
        }

        if ($totalAmortized > 0.00
            && false === $this->addLineToEntry($entry, $this->document->codsubcuentahaber, true, $totalAmortized))
        {
            return false;
        }

        if ($pendingAmortization > 0.00) {
            return $this->addLineToEntry($entry, $this->document->codsubcuentaperdidas, true, $pendingAmortization);
        }
        return true;
    }
    ...
    protected function getNewEntry()
    {
        ...
        $entry->fecha = $this->endDate;
        $entry->idempresa = $this->exercise->idempresa;
        $entry->importe = $this->document->valor;
        return $entry;
    }
    ...
    protected function initialCheck(): bool
    {
        ...
        if (false === empty($this->document->idasientofinvida)
            || false === empty($this->document->idfacturaventa))
        {
        ...
        if (empty($this->endDate) || false === strtotime($this->endDate)) {
            Tools::log()->info('date-end-amortization-error');
            return false;
        }

        if (false === $this->loadExerciseForDate($this->document->idempresa, $this->endDate)) {
    ...
    {
        return Tools::lang()->trans('end-useful-life') . ': '
