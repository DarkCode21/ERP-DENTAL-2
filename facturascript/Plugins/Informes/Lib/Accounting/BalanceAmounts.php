...
class BalanceAmounts
{
    ...
    public function generate(int $idcompany, string $dateFrom, string $dateTo, array $params = []): array
    {
		...
		$isselectedlevel = (bool)($params['isselectedlevel'] ?? false);
		...
		// Cuando level es 0, solo mostrar subcuentas (nivel 10)
        if ($level == 0) {
		...
		if ($isselectedlevel) {
            // Solo para isselectedlevel=true: calcular totales basados en las filas mostradas
            $debeTotal = $haberTotal = 0.00;

            // Sumar los valores de las filas mostradas
            foreach ($rows as $row) {
                $debe = strip_tags($row['debe']);
                $haber = strip_tags($row['haber']);
            ...
            $saldoTotal = $debeTotal - $haberTotal;

            $totals[0]['debe'] = $this->formatValue($debeTotal);
            $totals[0]['haber'] = $this->formatValue($haberTotal);
            $totals[0]['saldo'] = $this->formatValue($saldoTotal);
        } else {
