...
class Txt347Export
{
    ...
    protected static function checkCifNif(array $item): string
    {
        ...
        if (!empty($codpais) && false === self::isSpanish($codpais)) {
    ...
    {
        // toleramos códigos incorrectos frecuentes: ESP, ESPAÑA, ES, vacío
        $upper = strtoupper(trim($codpais));
        if (empty($upper) || in_array($upper, ['ES', 'ESP', 'ESPANA', 'ESPAÑA'], true)) {
            return true;
        }
