...
class Modelo347 extends Controller
{
    ...
    protected function downloadExcelAction(): void
    {
        ...
        $fileName = 'Modelo_347_' . $this->codejercicio . '.xlsx';
        ...
        $this->response->setContent($xlsExport->getDoc());
    }
    ...
    protected function getInvoiceSql(string $tableName, string $codeField): string
    {
        $sql = 'SELECT ' . $codeField . ', cifnif, EXTRACT(MONTH FROM fecha) as mes, sum(total + COALESCE(totalirpf, 0) - COALESCE(totalsuplidos, 0)) as total'
            . ' FROM ' . $tableName
            . ' INNER JOIN series ON ' . $tableName . '.codserie = series.codserie'
            . ' AND (COALESCE(series.siniva, false) = false OR series.codserie IN (\'EX\', \'EX2\'))'
            . $this->getInvoiceSqlWhere($tableName);

        // el filtro de IRPF sólo aplica a facturas de venta:
        // en facturas de compra siempre se declara el importe bruto (base + IVA)
        // y la fórmula ya añade de vuelta el totalirpf retenido.
        if ($this->excludeIrpf && $tableName === 'facturascli') {
