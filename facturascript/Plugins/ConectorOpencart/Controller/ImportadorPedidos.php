<?php
namespace FacturaScripts\Plugins\ConectorOpencart\Controller;
...
class ImportadorPedidos extends \FacturaScripts\Core\Lib\ExtendedController\PanelController{

    public function getPageData(): array
    {
        $pageData = parent::getPageData();
        $pageData['title'] = 'Importador pedidos';
        $pageData['menu'] = 'sales';
    ...
    protected function createViews() {
		$this->addHtmlView('Importador', 'ImportadorPedidos', 'PedidoCliente', 'order','fas fa-code-branch');
    }
	...
	protected function execPreviousAction($action) {
	   if($action === 'import') {
		   include(getcwd().'/Plugins/ConectorOpencart/config.php');
		   #echo "15/11/2022 - %d,%m,%Y = " . strftime("%d,%m,%Y", strtotime("2022-11-15")) . "\n";
		...
		//	$code=   $pedido->newCode();   
		//	   $pedido->loadFromCode('1');	   
		//$pedido->loadFromCode('', [ new DataBaseWhere('cifnif','72415847G') ]);
		//var_dump($pedido);
			   ...
			   */
			   ...
			   foreach($order->totals as $key => $total){
					switch($key){
						case 'sub_total':
							$pedido->neto		=		$total;
							$pedido->netosindto	=		$total;
							break;
						case 'total':
							$pedido->total		=		$total;
							$pedido->totaleuros =		$total;
							break;
						case 'tax':
							$tax	+=	$total;
							break;
	...
	*/		   
	//		   echo $pedido->neto . " + " . $suplidos . " + ". $tax . " = " . $pedido->total;
			   $pedido->totaliva		=		$tax;	
	//		   $pedido->totalsuplidos	=		$suplidos;	
				#echo var_dump($pedido);
				# exit;
			    $idpedido = $this->save($pedido);
			    #die(var_dump($idpedido));
				if (false === $idpedido) {
					#echo "Error en pedido <br>";
					#die(var_dump($pedido));
			   ...
			   foreach($order->order_products as $order_product){
				   ...
				   $lineapedido->referencia		= 	substr($referencia,0,30); 		
				   #(strlen($referencia)>30?substr($referencia,0,30):$referencia);
				   ...
				   // echo var_dump($lineapedido);
					if (false === $lineapedido->save()) {
						#echo "1. Error en lineapedido <br>";
						#die(var_dump($lineapedido));
						$this->toolBox()->i18nLog()->error('record-save-error');
						return true;
					}		   
			   }
			   ...
			   foreach($order_totals as $code => $value){
				   ...
				   //exit;
					...
					if (false === $lineapedido->save()) {
						#echo "2. Error en lineapedido <br>";
						#die(var_dump($lineapedido));
						$this->toolBox()->i18nLog()->error('record-save-error');
						return true;
					}
...
//die(var_dump($product));		   
		   ...
		   if($product->manufacturer_id){
			   ...
			   if(false === $fabricante->loadFromCode('', [ new DataBaseWhere('nombre', $product->mannufacturer) ])){
					 ...
					 if (false === $fabricante->save()) {	
						    #echo "1. Error en fabricante <br>";
							#die(var_dump($fabricante));
							$this->toolBox()->i18nLog()->error('record-save-error');
							return true;
					}
			   }
		   ...
		   $producto->referencia	=	$referencia; #substr($referencia,0,30);
		   ...
		   if ($flagRegCliente) {			   
			   ...
			   $contacto->codpais		=		$order->country;
				...
				if (false === $contacto->save()) {
				   #echo "1. Error en contacto <br>";
				   #die(var_dump($contacto));
					$this->toolBox()->i18nLog()->error('record-save-error');
					return true;
				}			   
		   } else {
	...
	private function createViewsImportador(){
	}
}
?>