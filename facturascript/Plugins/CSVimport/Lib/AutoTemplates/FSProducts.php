 ...
 */
class FSProducts implements AutoTemplateInterface
{
    ...
    public function isValid(string $filePath, string $profile): bool
    {
        ...
        foreach (range(0, 6) as $start) {
            ...
            if ($csv['titles'][0] === 'actualizado' && $csv['titles'][1] === 'bloqueado') {
                return true;
            }
        }
    ...
    protected function getConteo(?string $codalmacen = null): ConteoStock
    {
        if (empty($this->conteo)) {
            ...
            $this->conteo->observaciones = 'CSVimport FSProducts';
            $this->conteo->save();
        }
    ...
    protected function importStock(Producto $product, Variante $variant, array $row): void
    {
        ...
        if (Plugins::isEnabled('StockAvanzado')) {
            ...
            $this->getConteo($warehouse->codalmacen)->addLine(
                $variant->referencia,
                $variant->idproducto,
                CsvFileTools::formatFloat($row['stockfis'])
            );
            return;
        }
        ...
        if (false === $stock->loadFromCode('', $where)) {
            ...
            $stock->cantidad = CsvFileTools::formatFloat($row['stockfis']);
        } else {
    ...
    {
        $variant->precio = CsvFileTools::formatFloat($row['precio']);
        return $variant->save();
    }
