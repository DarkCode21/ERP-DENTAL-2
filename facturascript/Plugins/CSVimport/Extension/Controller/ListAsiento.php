...
class ListAsiento
{
    ...
    public function importAccountingEntriesAction(): Closure
    {
        return function () {
            ...
            if ($template === CsvFileTools::AUTOMATIC && $this->importAccountingEntriesAutoAction($filePath)) {
                return true;
            }
    ...
    public function importAccountingEntriesAutoAction(): Closure
    {
        return function (string $filePath) {
            // buscamos una plantilla automática
            $autoTemplates = CSVfile::autoTemplate($filePath, 'accounting-entries');
            ...
            $offset = (int)$this->request->get('import-offset', 0);
            $saveLines = (int)$this->request->get('save-lines', 0);
            if (false === $autoTemplates->run($filePath, 'accounting-entries', $mode, $offset, $saveLines)) {
                return true;
            }
