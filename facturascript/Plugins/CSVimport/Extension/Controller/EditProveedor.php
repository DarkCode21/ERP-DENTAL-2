...
class EditProveedor
{
    public function createViews(): Closure
    {
        return function () {
            ...
            if ($this->permissions->allowImport) {
                $this->addButton('ListProductoProveedor', [
                    'action' => 'upload-supplier-products',
    ...
    public function execPreviousAction(): Closure
    {
        return function ($action) {
            switch ($action) {
                case 'import-supplier-products':
                    $this->importSuppliersAction();
                    break;

                case 'upload-supplier-products':
    ...
    public function importSuppliersAction(): Closure
    {
        return function () {
            ...
            if ($template === CsvFileTools::NEW_TEMPLATE) {
                $newCsvFile = CSVfile::newTemplate($fileName, 'supplier-products');
                $newCsvFile->mode = $this->request->get('import-mode');
                $newCsvFile->codproveedor = $this->request->get('code');
                if ($newCsvFile->save()) {
            ...
            $templateModel = $template === CsvFileTools::AUTOMATIC ?
                ...
                CsvFileTools::getFileTemplate($filePath, $template, 'accounting-entries');
            ...
            if ($result['offset'] > 0 && $result['offset'] < $result['total']) {
                ...
                $this->redirect($model->url() . '&action=import-supplier-products&import-filename=' . urlencode($fileName)
    ...
    public function importSuppliersAutoAction(): Closure
    {
        return function (string $filePath) {
            ...
            $autoTemplates = CSVfile::autoTemplate($filePath, 'supplier-products');
            ...
            $offset = (int)$this->request->get('import-offset', 0);
            $saveLines = (int)$this->request->get('save-lines', 0);
            if (false === $autoTemplates->run($filePath, 'supplier-products', $mode, $offset, $saveLines)) {
                return true;
            }
            ...
            if ($autoTemplates->continue()) {
                ...
                Tools::log()->notice('importing');
                $fileName = $this->request->get('import-filename');
                $this->redirect($this->url() . '?action=import-supplier-products&import-filename=' . urlencode($fileName)
    ...
    public function loadData(): Closure
    {
        return function ($viewName, $view) {
            if ($viewName !== 'ListProductoProveedor') {
                return;
            }
            ...
            if ($column && $column->widget->getType() === 'select') {
                ...
                $where = [
                    ...
                    new DataBaseWhere('profile', 'supplier-products')
                ];
