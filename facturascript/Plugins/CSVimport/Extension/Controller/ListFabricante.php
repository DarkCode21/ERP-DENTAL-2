...
class ListFabricante
{
    ...
    public function importManufacturersAction(): Closure
    {
        return function () {
            ...
            if ($template === CsvFileTools::AUTOMATIC && $this->importManufacturersAutoAction($filePath)) {
                return true;
            }
            ...
            $templateModel = $template === CsvFileTools::AUTOMATIC ?
                ...
                CsvFileTools::getFileTemplate($filePath, $template, 'manufacturers');
    ...
    public function importManufacturersAutoAction(): Closure
    {
        return function (string $filePath) {
            // buscamos una plantilla automática
            $autoTemplates = CSVfile::autoTemplate($filePath, 'manufacturers');
            ...
            $offset = (int)$this->request->get('import-offset', 0);
            $saveLines = (int)$this->request->get('save-lines', 0);
            if (false === $autoTemplates->run($filePath, 'manufacturers', $mode, $offset, $saveLines)) {
                return true;
            }
