...
class ListCliente
{
    public function createViews(): Closure
    {
        return function () {
            ...
            if ($this->permissions->allowImport) {
                ...
                if (array_key_exists('ListContacto', $this->views)) {
                    $this->addButton('ListContacto', [
    ...
    public function importContactsAction(): Closure
    {
        return function () {
            ...
            if ($template === CsvFileTools::AUTOMATIC && $this->importContactsAutoAction($filePath)) {
                return true;
            }
            ...
            $templateModel = $template === CsvFileTools::AUTOMATIC ?
                ...
                CsvFileTools::getFileTemplate($filePath, $template, 'contacts');
    ...
    public function importContactsAutoAction(): Closure
    {
        return function (string $filePath) {
            // buscamos una plantilla automática
            $autoTemplates = CSVfile::autoTemplate($filePath, 'contacts');
            ...
            $offset = (int)$this->request->get('import-offset', 0);
            $saveLines = (int)$this->request->get('save-lines', 0);
            if (false === $autoTemplates->run($filePath, 'contacts', $mode, $offset, $saveLines)) {
                return true;
            }
    ...
    public function importCustomersAction(): Closure
    {
        return function () {
            ...
            // se ha elegido plantilla automática
            if ($template === CsvFileTools::AUTOMATIC && $this->importCustomersAutoAction($filePath)) {
                return true;
            }

            // se ha seleccionado una plantilla concreta
            $templateModel = $template === CsvFileTools::AUTOMATIC ?
                CsvFileTools::getFileTemplate($filePath) :
                CsvFileTools::getFileTemplate($filePath, $template, 'customers');
            if (is_null($templateModel)) {
                Tools::log()->warning('template-not-found');
    ...
    public function importCustomersAutoAction(): Closure
    {
        return function (string $filePath) {
            // buscamos una plantilla automática
            $autoTemplates = CSVfile::autoTemplate($filePath, 'customers');
            if ($autoTemplates === null) {
                return false;
            }
            ...
            $offset = (int)$this->request->get('import-offset', 0);
            $saveLines = (int)$this->request->get('save-lines', 0);
            if (false === $autoTemplates->run($filePath, 'customers', $mode, $offset, $saveLines)) {
                return true;
            }
    ...
    public function loadData(): Closure
    {
        return function ($viewName, $view) {
            if (false === in_array($viewName, ['ListCliente', 'ListContacto'])) {
                return;
            }
