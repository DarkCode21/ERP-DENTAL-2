...
class ListProducto
{
    ...
    public function importProductsAction(): Closure
    {
        return function () {
            ...
            if ($template === CsvFileTools::AUTOMATIC && $this->importProductsAutoAction($filePath)) {
                return true;
            }
            ...
            $templateModel = $template === CsvFileTools::AUTOMATIC ?
                ...
                CsvFileTools::getFileTemplate($filePath, $template, 'products');
    ...
    public function importProductsAutoAction(): Closure
    {
        return function (string $filePath) {
            // buscamos una plantilla automática
            $autoTemplates = CSVfile::autoTemplate($filePath, 'products');
            ...
            $offset = (int)$this->request->get('import-offset', 0);
            $saveLines = (int)$this->request->get('save-lines', 0);
            if (false === $autoTemplates->run($filePath, 'products', $mode, $offset, $saveLines)) {
                return true;
            }
    ...
    public function importVariantsAction(): Closure
    {
        return function () {
            ...
            // se ha elegido plantilla automática
            if ($template === CsvFileTools::AUTOMATIC && $this->importVariantsAutoAction($filePath)) {
                return true;
            }
            ...
            $templateModel = $template === CsvFileTools::AUTOMATIC ?
                CsvFileTools::getFileTemplate($filePath) :
                CsvFileTools::getFileTemplate($filePath, $template, 'variants');
            if (is_null($templateModel)) {
                Tools::log()->warning('template-not-found');
    ...
    public function importVariantsAutoAction(): Closure
    {
        return function (string $filePath) {
            // buscamos una plantilla automática
            $autoTemplates = CSVfile::autoTemplate($filePath, 'variants');
            if ($autoTemplates === null) {
                return false;
            }
            ...
            $offset = (int)$this->request->get('import-offset', 0);
            $saveLines = (int)$this->request->get('save-lines', 0);
            if (false === $autoTemplates->run($filePath, 'variants', $mode, $offset, $saveLines)) {
                return true;
            }
