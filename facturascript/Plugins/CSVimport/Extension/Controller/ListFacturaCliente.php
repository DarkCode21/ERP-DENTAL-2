...
class ListFacturaCliente
{
    ...
    public function importInvoicesAction(): Closure
    {
        return function () {
            ...
            if ($template === CsvFileTools::AUTOMATIC && $this->importInvoicesAutoAction($filePath)) {
                return true;
            }

            // se ha seleccionado una plantilla concreta, o una automática
            $templateModel = $template === CsvFileTools::AUTOMATIC ?
                CsvFileTools::getFileTemplate($filePath) :
                CsvFileTools::getFileTemplate($filePath, $template, 'customer-invoices');
    ...
    public function importInvoicesAutoAction(): Closure
    {
        return function (string $filePath) {
            // buscamos una plantilla automática
            $autoTemplates = CSVfile::autoTemplate($filePath, 'customer-invoices');
            ...
            $offset = (int)$this->request->get('import-offset', 0);
            $saveLines = (int)$this->request->get('save-lines', 0);
            if (false === $autoTemplates->run($filePath, 'customer-invoices', $mode, $offset, $saveLines)) {
                return true;
            }
            ...
            if ($autoTemplates->continue()) {
                ...
                $fileName = $this->request->get('import-filename');
                $this->redirect($this->url() . '?action=import-invoices&import-filename=' . urlencode($fileName)
                    . '&import-offset=' . '&import-offset=' . $offset . '&save-lines=' . $saveLines
