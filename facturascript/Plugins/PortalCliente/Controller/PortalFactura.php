...
class PortalFactura extends PortalViewController
{
    ...
    {
        $this->addListView($viewName, 'FacturaCliente', 'refunds', 'fas fa-share-square')
    ...
    protected function loadData($viewName, $view)
    {
        switch ($viewName) {
            ...
            case 'ListPortalFactura':
                $where = [new DataBaseWhere('idfacturarect', $this->views['main']->model->primaryColumnValue())];
                $view->loadData('', $where);
                $view->setSettings('active', $view->count > 0);
                break;

            case 'ListReciboCliente':
                $where = [new DataBaseWhere('idfactura', $this->views['main']->model->primaryColumnValue())];
                $view->loadData('', $where);
                break;
