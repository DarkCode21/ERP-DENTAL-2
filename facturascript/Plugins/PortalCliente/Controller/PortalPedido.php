...
class PortalPedido extends PortalViewController
{
    ...
    public function getImages(LineaPedidoCliente $line): string
    {
    ...
    protected function cancelAction(): bool
    {
        ...
        if (false === $model->exists()) {
            ...
            return true;
        } elseif (false === $status->loadFromCode(Tools::settings('portalcliente', 'status_cancel_orders'))) {
            Tools::log()->error('status-cancel-orders-not-configured');
            return true;
        }
        ...
        if ($model->save()) {
            WorkQueue::send(
                ...
                [
                    ...
                    'notice_name' => 'group_cancel_orders',
                ]
            );
