...
class PurchasesHeaderHTMLMod implements PurchasesModInterface
{
    ...
    private function siiSent(Translator $i18n, PurchaseDocument $model): string
    {
        $value = empty($model->sii_sent) ? '' : date('Y-m-d', strtotime($model->sii_sent));
        return empty($model->subjectColumnValue()) || $model->modelClassName() !== 'FacturaProveedor'
            ? ''
            : '<div class="col-sm-6">'
    ...
    private function siiStatus(Translator $i18n, PurchaseDocument $model): string
    {
        return empty($model->subjectColumnValue()) || $model->modelClassName() !== 'FacturaProveedor'
            ? ''
            : '<div class="col-sm-6">'
