...
final class Signature
{
    public static function generate(VerifactuRegistroFactura $regInvoice): bool
    {
        ...
        if (false === openssl_pkcs12_read($certData, $certs, $company->vf_password)) {
            $errorMessage = 'Error al leer el certificado Verifactu de la empresa ' . $company->nombrecorto . ': ' . openssl_error_string();
            ...
            VerifactuErrorLog::logError(
                $company->idempresa,
                'verifactu-signature-generation',
                'CERTIFICATE_ERROR',
                $errorMessage,
                [
                    'company_name' => $company->nombre,
                    'company_short_name' => $company->nombrecorto,
                    'factura_id' => $regInvoice->id ?? null,
                    'json_file' => $regInvoice->file_json ?? null,
                    'openssl_error' => openssl_error_string(),
                    'step' => 'certificate_read'
                ]
            );
