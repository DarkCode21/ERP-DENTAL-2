...
use FacturaScripts\Core\Controller\ListFacturaProveedor as LFP;
 ...
 */
class ListFsRptLibroGastos extends LFP {
    ...
    public function __construct(string $className, string $uri = '')
    {
        ...
        $this->baseLibro = new BaseLibrosFacturas('facturasprov', 'lineasfacturasprov');

        $company = new Model\Empresa();
        if ($company->loadFromCode($code)) {
            $companyName = $company->nombre;
        }
        $this->params = ['title' => 'Libro de facturas de gastos',
    ...
    public function getPageData(): array {
        ...
        $pagedata['submenu'] = 'purchases';
        return $pagedata;
    }

     protected function createViews() {
        // listado de facturas de proveedor
        $this->createViewPurchases('ListFsRptLibroGastos', 'FacturaProveedor', 'invoices');
    }
...
case 'print-screen':
   ...
   public function exportReport() {
    ...
    public function generateReport1()
    {
        ...
        $this->setTemplate('ReportLibroIngresos');
    }

    private function renderHtml(string $template, string $controllerName = '')
    {
        $templateVars = [
            'controllerName' => 'ListFsRptLibroGastos',
            'debugBarRender' => true,
            'fsc' => $this,
            'template' => $template,
        ];
